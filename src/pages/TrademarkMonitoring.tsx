...
import { ChevronRight, Send, Loader2, Bot, User, Sparkles, Trash2, Globe2, AlertCircle, FileCheck, RefreshCw, Shield } from 'lucide-react'
...
const EXAMPLE_QUERIES = [
  "帮我设置'IPClaw'品牌的全球商标监测，重点关注东南亚市场",
  "发现有人在海外抢注了我司商标，请给出应对方案",
  "帮我建立商标使用证据管理体系，防范撤三风险",
  "分析我司10件商标的续展情况，制定续展计划",
]

const CAPABILITIES = [
  { icon: Globe2, label: '全球监测', description: '196国家地区', color: '#F97316' },
  { icon: AlertCircle, label: '抢注预警', description: '一带一路重点', color: '#EF4444' },
  { icon: FileCheck, label: '证据管理', description: '防撤三体系', color: '#22C55E' },
  { icon: RefreshCw, label: '续展提醒', description: '12+6月窗口', color: '#3B82F6' },
  { icon: Shield, label: '维权支持', description: '行政司法双轨', color: '#8B5CF6' },
]
...
export default function TrademarkMonitoring() {
  ...
  const sendMessage = async (content: string) => {
    ...
    try {
      ...
      const response = await fetch('/api/trademark-monitoring-agent', {
        method: 'POST',
      ...
      <div className="flex flex-col h-[calc(100vh-56px)]">
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-[var(--navy-700)]">
          <div className="flex items-center gap-2 text-sm">
            ...
            <ChevronRight size={14} className="text-[var(--navy-600)]" />
            <span className="text-[var(--text-primary)] font-medium">商标监测</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs" style={{ background: 'rgba(249,115,22,0.1)', color: '#F97316', border: '1px solid rgba(249,115,22,0.2)' }}>
        ...
        <div className="flex-1 overflow-y-auto">
          {!hasMessages ? (
            ...
            <div className="flex flex-col items-center justify-center h-full px-6">
              ...
              >
                <div className="w-16 h-16 rounded-2xl mx-auto mb-5 flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #F97316, #F59E0B)' }}>
                ...
                <p className="text-sm text-[var(--text-secondary)] mb-8">
                  基于中华商标协会国际监测预警体系，提供商标抢注监测、侵权监控、使用证据管理、续展提醒全生命周期守护
                </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {EXAMPLE_QUERIES.map((query, i) => (
                    <motion.button
                      ...
                      className="text-left p-3.5 rounded-xl border border-[var(--navy-700)] text-sm text-[var(--text-secondary)] hover:border-[#F97316] hover:text-[var(--text-primary)] transition-all duration-200 hover:bg-[rgba(249,115,22,0.03)]"
                      style={{ background: 'linear-gradient(180deg, #1E293B 0%, #0F172A 100%)' }}
                    >
                      <Sparkles size={14} className="text-[#F97316] mb-1.5" />
                      {query}
                    </motion.button>
                  ))}
                </div>
            ...
            <div className="max-w-3xl mx-auto px-6 py-6">
              ...
              {loading && (
                ...
                >
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 bg-gradient-to-br from-orange-500 to-amber-500">
            ...
            <div className="flex items-end gap-3">
              ...
              <div className="flex-1 relative">
                <textarea
                  ...
                  placeholder="输入您的监测需求，例如：设置商标监测、查询侵权情况..."
                  rows={1}
                  className="w-full px-4 py-3 pr-12 rounded-xl border border-[var(--navy-700)] bg-[#0F172A] text-sm text-[var(--text-primary)] placeholder:text-[var(--navy-600)] focus:outline-none focus:border-[#F97316] resize-none transition-colors"
              ...
              <button
                ...
                style={{
                  background: input.trim() && !loading ? 'linear-gradient(135deg, #F97316, #F59E0B)' : 'var(--navy-700)',
                  color: 'white',
                }}
              >

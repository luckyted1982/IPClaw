...
import { ChevronRight, Send, Search, Loader2, Bot, User, Sparkles, Trash2, ShieldCheck, Tag, AlertTriangle, FileText, Globe, Database, ExternalLink } from 'lucide-react'
...
const EXAMPLE_QUERIES = [
  "帮我检索'智云科技'这个商标在第9类电子产品的注册可行性",
  "我想注册一个食品品牌商标，名字叫'味来可期'，请帮我做全面检索",
  "分析'小确幸'这个商标名称的显著性和注册风险",
  "我司计划推出新能源汽车品牌，请推荐合适的商标名称并评估",
]

const CAPABILITIES = [
  { icon: Search, label: '相似检索', description: '音形义三维比对', color: '#3B82F6' },
  { icon: ShieldCheck, label: '风险评估', description: '高中低三级评估', color: '#22C55E' },
  { icon: Tag, label: '分类建议', description: '尼斯45类智能匹配', color: '#F59E0B' },
  { icon: AlertTriangle, label: '禁用审查', description: '8类法定禁用项', color: '#EF4444' },
  { icon: FileText, label: '分析报告', description: '结构化输出', color: '#8B5CF6' },
]
...
const SEARCH_CHANNELS = [
  {
    ...
    color: '#22C55E',
    items: [
      { name: '中国商标网', url: 'https://wcjs.sbj.cnipa.gov.cn', desc: '商标局官方查询系统，覆盖1980年至今全部商标数据' },
      { name: '商标数据开放系统', url: 'https://sbj.cnipa.gov.cn', desc: '全量数据批量下载，支持API接口调用' },
      { name: '国家知识产权公共服务平台', url: 'https://ggfw.cnipa.gov.cn', desc: '一站式知识产权服务入口' },
    ]
  },
  {
    category: '国内第三方平台',
    ...
    color: '#3B82F6',
    items: [
      { name: '阿里云商标检索', url: 'https://tm.aliyun.com', desc: '红黄绿三色风险标注，45类一键查询' },
      { name: '标库网', url: 'https://www.tmkoo.com', desc: '按类别统计，关联企业工商信息' },
      { name: '白兔商标查询网', url: 'https://so.tmo.cn', desc: '专业近似匹配，数据更新及时' },
      { name: '擎策·知海数据库', url: '', desc: '国内9667万国际1.06亿条，覆盖127国' },
    ]
  },
  {
    category: '国际商标检索',
    ...
    color: '#8B5CF6',
    items: [
      { name: 'WIPO Global Brand Database', url: 'https://branddb.wipo.int', desc: '50+ million records，覆盖50+数据库' },
      { name: 'TMview', url: 'https://www.tmdn.org/tmview', desc: '44+ million trade marks，来自67个商标局' },
      { name: 'EUIPO eSearch Plus', url: 'https://euipo.europa.eu', desc: '欧盟商标查询，Similarity Check功能' },
      { name: 'USPTO Trademark Search', url: 'https://tmsearch.uspto.gov', desc: '美国商标查询，替代原TESS系统' },
    ]
  },
]
...
export default function TrademarkSearch() {
  ...
  const sendMessage = async (content: string) => {
    ...
    try {
      ...
      const response = await fetch('/api/trademark-search-agent', {
        method: 'POST',
  ...
  return (
    <Layout rightPanelModule="trademark">
      <div className="flex flex-col h-[calc(100vh-56px)]">
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-[var(--navy-700)]">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="text-[var(--navy-600)] cursor-pointer hover:text-[var(--gold-400)]" onClick={() => navigate('/trademark')}>商标业务</span>
            <ChevronRight size={14} className="text-[var(--navy-600)]" />
            <span className="text-[var(--text-primary)] font-medium">商标检索</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs" style={{ background: 'rgba(59,130,246,0.1)', color: '#3B82F6', border: '1px solid rgba(59,130,246,0.2)' }}>
        ...
        <div className="flex-1 overflow-y-auto">
          {!hasMessages ? (
            ...
            <div className="flex flex-col items-center justify-center h-full px-6">
              ...
              >
                <div className="w-16 h-16 rounded-2xl mx-auto mb-5 flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #3B82F6, #06B6D4)' }}>
                ...
                <p className="text-sm text-[var(--text-secondary)] mb-8">
                  基于《商标审查审理指南》及尼斯分类标准，提供相似商标检索、在先权利排查、显著性评估、分类建议全流程服务
                </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {EXAMPLE_QUERIES.map((query, i) => (
                    <motion.button
                      ...
                      className="text-left p-3.5 rounded-xl border border-[var(--navy-700)] text-sm text-[var(--text-secondary)] hover:border-[#3B82F6] hover:text-[var(--text-primary)] transition-all duration-200 hover:bg-[rgba(59,130,246,0.03)]"
                      style={{ background: 'linear-gradient(180deg, #1E293B 0%, #0F172A 100%)' }}
                    >
                      <Sparkles size={14} className="text-[#3B82F6] mb-1.5" />
                      {query}
                    </motion.button>
                  ))}
                </div>
            ...
            <div className="max-w-3xl mx-auto px-6 py-6">
              ...
              {loading && (
                ...
                >
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 bg-gradient-to-br from-blue-500 to-cyan-500">
            ...
            <div className="flex items-end gap-3">
              ...
              <div className="flex-1 relative">
                <textarea
                  ...
                  placeholder="输入您的商标检索需求，例如：查询某商标的近似情况、评估注册可行性..."
                  rows={1}
                  className="w-full px-4 py-3 pr-12 rounded-xl border border-[var(--navy-700)] bg-[#0F172A] text-sm text-[var(--text-primary)] placeholder:text-[var(--navy-600)] focus:outline-none focus:border-[#3B82F6] resize-none transition-colors"
              ...
              <button
                ...
                style={{
                  background: input.trim() && !loading ? 'linear-gradient(135deg, #3B82F6, #06B6D4)' : 'var(--navy-700)',
                  color: 'white',
                }}
              >

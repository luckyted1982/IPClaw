...
import { ChevronRight, Send, Loader2, Bot, User, Sparkles, Trash2, LockKeyhole, Landmark, Handshake, Calculator, ShieldAlert, FileText, Banknote, BookOpen } from 'lucide-react'
...
const EXAMPLE_QUERIES = [
  "我们公司的发明专利能质押多少贷款？质押率一般多少？",
  "知识产权质押融资的登记流程是怎样的？在哪里办理？",
  "各地对IP质押融资有什么补贴政策？深圳最高能补贴多少？",
  "哪家银行的IP质押贷款产品利率最低？准入条件是什么？",
]

const CAPABILITIES = [
  { icon: LockKeyhole, label: '质押登记', description: '一网通办指导', color: '#F59E0B' },
  { icon: Landmark, label: '银行对接', description: '20家银行产品', color: '#EF4444' },
  { icon: Handshake, label: '方案设计', description: '融资金额期限', color: '#22C55E' },
  { icon: Calculator, label: '补贴测算', description: '15省市政策', color: '#8B5CF6' },
  { icon: ShieldAlert, label: '风控审查', description: '四维风险评估', color: '#06B6D4' },
]
...
const POLICY_HIGHLIGHTS = [
  {
    title: '质押融资工作通知',
    date: '银保监发2019号',
    content: '鼓励银行业金融机构开展知识产权质押融资，建立风险补偿机制和贴息政策',
    color: '#F59E0B',
  },
  {
    title: '一网通办平台',
    date: '国家知识产权局',
    content: '在线办理专利/商标/著作权质押登记，5个工作日内完成',
    color: '#22C55E',
  },
  {
    title: '深圳补贴政策',
    date: '2024年最新',
    content: '最高补贴50%利息+100%评估费，单家企业年度上限500万元',
    color: '#8B5CF6',
  },
]

export default function IPPledge() {
  ...
  const sendMessage = async (content: string) => {
    ...
    try {
      ...
      const response = await fetch('/api/ip-pledge-agent', {
        method: 'POST',
      ...
      <div className="flex flex-col h-[calc(100vh-56px)]">
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-[var(--navy-700)]">
          <div className="flex items-center gap-2 text-sm">
            ...
            <ChevronRight size={14} className="text-[var(--navy-600)]" />
            <span className="text-[var(--text-primary)] font-medium">质押融资</span>
          </div>
            ...
            <div className="flex flex-col items-center justify-center h-full px-6">
              ...
              >
                ...
                <p className="text-sm text-[var(--text-secondary)] mb-8">
                  专利/商标质押融资方案设计与银行对接，覆盖质押登记、评估报告、补贴申请全流程服务
                </p>
        ...
        <div className="border-t border-[var(--navy-700)] px-6 py-4">
          <div className="max-w-3xl mx-auto">
            <div className="flex items-end gap-3">
              ...
              <div className="flex-1 relative">
                <textarea
                  ...
                  placeholder="输入您的质押融资需求，例如：咨询质押贷款额度、了解登记流程、查询补贴政策..."
                  rows={1}
            ...
            <div className="flex items-center justify-between mt-2 text-xs text-[var(--navy-600)]">
              ...
              <span>质押登记 | 银行对接 | 补贴政策 | 一网通办 | 风险补偿</span>
            </div>
          </div>
        </div>
      </div>

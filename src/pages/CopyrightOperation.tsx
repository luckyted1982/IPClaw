...
import { ChevronRight, Send, Loader2, Bot, User, Sparkles, Trash2, TrendingUp, Handshake, Landmark, Globe, ExternalLink, Coins, Users } from 'lucide-react'
...
const EXAMPLE_QUERIES = [
  "我们公司有200多个软件著作权，如何进行版权资产估值和融资？",
  "想把一部小说IP改编成影视游戏，许可模式怎么设计？怎么定价？",
  "数据资产入表新政下，版权数据如何作为无形资产入表？",
  "如何通过音著协/文著协等集体管理组织获取版税收入？",
]

const CAPABILITIES = [
  { icon: TrendingUp, label: '价值挖掘', description: '版权资产估值', color: '#A855F7' },
  { icon: Handshake, label: '交易撮合', description: '转让/许可设计', color: '#EC4899' },
  { icon: Coins, label: '版权金融', description: '质押/证券化', color: '#8B5CF6' },
  { icon: Users, label: '集体管理', description: '五大协会对接', color: '#6366F1' },
  { icon: Landmark, label: '国际布局', description: '条约体系运用', color: '#A855F7' },
]
...
const OPERATION_CHANNELS = [
  {
    ...
    color: '#A855F7',
    items: [
      { name: '中国国际版权博览会', url: '', desc: '国家级龙头展会，年度举办' },
      { name: '国家版权交易中心', url: '', desc: '全国多地分支机构，综合交易' },
      { name: '深圳文化产权交易所', url: '', desc: '版权金融创新，ABS/质押' },
    ]
  },
  {
    category: '集体管理组织',
    ...
    color: '#8B5CF6',
    items: [
      { name: '音著协(MCSC)', url: 'http://www.mcsc.com.cn', desc: '音乐词曲KTV/网络音乐' },
      { name: '文著协(WCSC)', url: 'http://www.prccopyright.org.cn', desc: '文字作品教科书/转载' },
      { name: '影著协/摄著协/音集协', url: '', desc: '电影/摄影/音像制品' },
    ]
  },
  {
    category: '国际条约与出海',
    ...
    color: '#6366F1',
    items: [
      { name: '伯尔尼公约(181国)', url: '', desc: '自动保护，无需登记' },
      { name: 'WIPO仲裁中心', url: '', desc: '国际版权争端解决' },
      { name: '北京条约/马拉喀什条约', url: '', desc: '视听表演者/阅读障碍便利' },
    ]
  },
]

export default function CopyrightOperation() {
  ...
  const sendMessage = async (content: string) => {
    ...
    try {
      ...
      const response = await fetch('/api/copyright-operation-agent', {
        method: 'POST',
      ...
      <div className="flex flex-col h-[calc(100vh-56px)]">
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-[var(--navy-700)]">
          <div className="flex items-center gap-2 text-sm">
            ...
            <ChevronRight size={14} className="text-[var(--navy-600)]" />
            <span className="text-[var(--text-primary)] font-medium">版权运营</span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {!hasMessages ? (
            ...
            <div className="flex flex-col items-center justify-center h-full px-6">
              ...
              >
                <div className="w-16 h-16 rounded-2xl mx-auto mb-5 flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #A855F7, #F59E0B)' }}>
                ...
                <p className="text-sm text-[var(--text-secondary)] mb-8">
                  基于超10万亿元版权产业规模，提供版权交易、许可授权、质押融资、集体管理、国际布局及数据资产入表全链条运营方案
                </p>
                ...
                >
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {OPERATION_CHANNELS.map((channel, i) => (
                      <div
                        key={i}
            ...
            <div className="max-w-3xl mx-auto px-6 py-6">
              ...
              {loading && (
                ...
                >
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 bg-gradient-to-br from-purple-500 to-amber-500">
            ...
            <div className="flex items-end gap-3">
              ...
              <div className="flex-1 relative">
                <textarea
                  ...
                  placeholder="输入您的版权运营需求，例如：版权估值、交易设计、融资方案、数据资产入表..."
                  rows={1}
              ...
              <button
                ...
                style={{
                  background: input.trim() && !loading ? 'linear-gradient(135deg, #A855F7, #F59E0B)' : 'var(--navy-700)',
                  color: 'white',
                }}
              >

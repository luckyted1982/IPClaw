...
import { ChevronRight, Send, Loader2, Bot, User, Sparkles, Trash2, ScanEye, AlertTriangle, Shield, Radar, ExternalLink, Globe } from 'lucide-react'
...
const EXAMPLE_QUERIES = [
  "我的短视频在抖音被大量搬运，如何进行全网监测？",
  "想对公司的品牌图片进行侵权监测，覆盖哪些平台比较好？",
  "'剑网2025'专项行动重点整治什么？我的情况适用吗？",
  "如何使用数字水印技术保护我们的原创内容？成本多少？",
]

const CAPABILITIES = [
  { icon: Radar, label: '全网监测', description: '48+平台覆盖', color: '#A855F7' },
  { icon: ScanEye, label: '智能识别', description: '多模态指纹比对', color: '#EC4899' },
  { icon: AlertTriangle, label: '实时预警', description: '秒级推送通知', color: '#8B5CF6' },
  { icon: Shield, label: '区块链存证', description: '司法级证据固定', color: '#6366F1' },
  { icon: Globe, label: '国际监测', description: '海外平台覆盖', color: '#A855F7' },
]
...
const MONITORING_CHANNELS = [
  {
    ...
    color: '#A855F7',
    items: [
      { name: '抖音/TikTok', url: '', desc: '短视频搬运监测，月活8亿' },
      { name: '哔哩哔哩(B站)', url: '', desc: 'UP主二次创作/鬼畜/字幕组监测' },
      { name: '快手/小红书/视频号', url: '', desc: '短视频/图文内容全网扫描' },
    ]
  },
  {
    category: '电商平台',
    ...
    color: '#8B5CF6',
    items: [
      { name: '淘宝/天猫/京东', url: '', desc: '盗版商品/假货/未授权周边监测' },
      { name: '拼多多/闲鱼', url: '', desc: '二手盗版/低价仿冒品追踪' },
      { name: '阿里知产保护平台', url: 'https://ipp.alibabagroup.com', desc: '官方投诉维权通道' },
    ]
  },
  {
    category: '国际平台',
    ...
    color: '#6366F1',
    items: [
      { name: 'YouTube Content ID', url: '', desc: '自动检测侵权视频并变现' },
      { name: 'Amazon Brand Registry', url: '', desc: '品牌保护+侵权商品下架' },
      { name: 'TikTok Copyright', url: '', desc: '全球短视频版权保护' },
    ]
  },
]

export default function CopyrightMonitoring() {
  ...
  const sendMessage = async (content: string) => {
    ...
    try {
      ...
      const response = await fetch('/api/copyright-monitoring-agent', {
        method: 'POST',
      ...
      <div className="flex flex-col h-[calc(100vh-56px)]">
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-[var(--navy-700)]">
          <div className="flex items-center gap-2 text-sm">
            ...
            <ChevronRight size={14} className="text-[var(--navy-600)]" />
            <span className="text-[var(--text-primary)] font-medium">版权监测</span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {!hasMessages ? (
            ...
            <div className="flex flex-col items-center justify-center h-full px-6">
              ...
              >
                <div className="w-16 h-16 rounded-2xl mx-auto mb-5 flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #A855F7, #6366F1)' }}>
                ...
                <p className="text-sm text-[var(--text-secondary)] mb-8">
                  基于"剑网2025"专项行动部署，提供7×24全网多模态侵权监测、数字水印/指纹识别、区块链存证一体化解决方案
                </p>
                ...
                >
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {MONITORING_CHANNELS.map((channel, i) => (
                      <div
                        key={i}
            ...
            <div className="max-w-3xl mx-auto px-6 py-6">
              ...
              {loading && (
                ...
                >
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 bg-gradient-to-br from-purple-500 to-indigo-500">
            ...
            <div className="flex items-end gap-3">
              ...
              <div className="flex-1 relative">
                <textarea
                  ...
                  placeholder="输入您的版权监测需求，例如：制定全网监测方案、分析侵权处置策略..."
                  rows={1}
              ...
              <button
                ...
                style={{
                  background: input.trim() && !loading ? 'linear-gradient(135deg, #A855F7, #6366F1)' : 'var(--navy-700)',
                  color: 'white',
                }}
              >

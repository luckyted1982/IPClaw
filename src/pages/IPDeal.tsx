...
import { ChevronRight, Send, Loader2, Bot, User, Sparkles, Trash2, ArrowLeftRight, Building2, Scale, FileText, Search, Globe, CheckCircle, Handshake, BookOpen } from 'lucide-react'
...
const EXAMPLE_QUERIES = [
  "我们想出售一批闲置专利，应该走拍卖还是协议转让？定价怎么做？",
  "独占许可和排他许可有什么区别？许可费率怎么谈？",
  "购买IP资产时需要做哪些尽职调查？怎么避免买到有纠纷的IP？",
  "跨境IP交易需要注意什么？外汇管制和税务问题怎么处理？",
]

const CAPABILITIES = [
  { icon: ArrowLeftRight, label: '交易撮合', description: '买卖双方对接', color: '#F59E0B' },
  { icon: Scale, label: '价格发现', description: '评估/拍卖/招标', color: '#EF4444' },
  { icon: Search, label: '尽调审查', description: '权属有效性核实', color: '#22C55E' },
  { icon: Globe, label: '跨境交易', description: '外汇税务合规', color: '#8B5CF6' },
  { icon: FileText, label: '合同设计', description: '核心条款模板', color: '#06B6D4' },
]
...
const POLICY_HIGHLIGHTS = [
  {
    title: '国有资产交易办法',
    date: '财政部令第32号',
    content: '国企IP交易需通过产权交易所公开挂牌，防止国有资产流失',
    color: '#F59E0B',
  },
  {
    title: '中国技术交易所',
    date: '国家级平台',
    content: '科技部与北京市政府共建，年交易额700亿元，IP交易核心平台',
    color: '#22C55E',
  },
  {
    title: '典型交易案例',
    date: '2024年',
    content: '某通信企业专利包以12.8亿元成交，创国内IP转让金额纪录',
    color: '#8B5CF6',
  },
]

export default function IPDeal() {
  ...
  const sendMessage = async (content: string) => {
    ...
    try {
      ...
      const response = await fetch('/api/ip-deal-agent', {
        method: 'POST',
      ...
      <div className="flex flex-col h-[calc(100vh-56px)]">
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-[var(--navy-700)]">
          <div className="flex items-center gap-2 text-sm">
            ...
            <ChevronRight size={14} className="text-[var(--navy-600)]" />
            <span className="text-[var(--text-primary)] font-medium">交易撮合</span>
          </div>
            ...
            <div className="flex flex-col items-center justify-center h-full px-6">
              ...
              >
                ...
                <p className="text-sm text-[var(--text-secondary)] mb-8">
                  知识产权买卖撮合、许可谈判、价格发现与交易结构设计，覆盖转让、独占许可、排他许可等多种交易模式
                </p>
            ...
            <div className="flex items-end gap-3">
              ...
              <div className="flex-1 relative">
                <textarea
                  ...
                  placeholder="输入您的IP交易需求，例如：咨询专利转让定价、许可模式选择、跨境交易合规..."
                  rows={1}

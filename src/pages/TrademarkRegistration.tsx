...
import { ChevronRight, Send, Loader2, Bot, User, Sparkles, Trash2, LayoutGrid, ClipboardList, Calculator, Clock } from 'lucide-react'
...
const EXAMPLE_QUERIES = [
  "我要为一家AI教育公司注册商标，公司名叫'智学未来'，请给出完整注册方案",
  "帮我为一个新式茶饮品牌起名并制定商标注册策略",
  "我司已有中文商标'星辰大海'，需要同步注册英文商标和国际商标",
  "评估在第25类服装鞋帽注册'极简生活'的风险，并给出备选方案",
]

const CAPABILITIES = [
  { icon: Sparkles, label: '智能命名', description: '独创性+显著性', color: '#22C55E' },
  { icon: LayoutGrid, label: '分类布局', description: '核心+关联+防御', color: '#3B82F6' },
  { icon: ClipboardList, label: '材料准备', description: '完整清单+模板', color: '#F59E0B' },
  { icon: Calculator, label: '费用预算', description: '官费+代理费', color: '#8B5CF6' },
  { icon: Clock, label: '时间规划', description: '8-10月全周期', color: '#EF4444' },
]
...
export default function TrademarkRegistration() {
  ...
  const sendMessage = async (content: string) => {
    ...
    try {
      ...
      const response = await fetch('/api/trademark-registration-agent', {
        method: 'POST',
      ...
      <div className="flex flex-col h-[calc(100vh-56px)]">
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-[var(--navy-700)]">
          <div className="flex items-center gap-2 text-sm">
            ...
            <ChevronRight size={14} className="text-[var(--navy-600)]" />
            <span className="text-[var(--text-primary)] font-medium">商标注册</span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {!hasMessages ? (
            ...
            <div className="flex flex-col items-center justify-center h-full px-6">
              ...
              >
                <div className="w-16 h-16 rounded-2xl mx-auto mb-5 flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #22C55E, #10B981)' }}>
                ...
                <p className="text-sm text-[var(--text-secondary)] mb-8">
                  基于《商标法》2025最新规定，提供商标命名、分类选择、材料准备、费用预算、时间规划一站式服务
                </p>
        ...
        <div className="border-t border-[var(--navy-700)] px-6 py-4">
          <div className="max-w-3xl mx-auto">
            <div className="flex items-end gap-3">
              ...
              <div className="flex-1 relative">
                <textarea
                  ...
                  placeholder="描述您的商标注册需求，例如：为新品牌设计商标名称、选择注册类别..."
                  rows={1}
              ...
              <button
                ...
                style={{
                  background: input.trim() && !loading ? 'linear-gradient(135deg, #22C55E, #10B981)' : 'var(--navy-700)',
                  color: 'white',
                }}
              >
            ...
            <div className="flex items-center justify-between mt-2 text-xs text-[var(--navy-600)]">
              ...
              <span>《商标法》第8-33条 | 尼斯分类 | 全程8-10月</span>
            </div>
          </div>
        </div>
      </div>
